...
import { blockGuidance } from "@/lib/cancel/blockGuidance";
import { getMacroSteps } from "@/lib/cancel/macro";

export const dynamic = "force-dynamic";
export const maxDuration = 120;

// Preview the cancellation plan (playbook / AI-researched) without acting,
// plus any recorded macro and money-blocking fallback guidance.
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ plan, blockGuidance: blockGuidance(plan), hasMacro: !!macro, macroSteps: macro?.length ?? 0 });
}
